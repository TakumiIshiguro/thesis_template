...
また，ロボットがシナリオの道順に沿って分岐路で適切な経路を選択する様子が確認できた．
...
学習時は角から $1 \sim 2$ m ほど前方で目標方向を与えているのに対し，テスト時では 角から $0.2 \sim 0.5$ m まで近づいてから与えられていることがわかった．
経路追従モジュールに目標方向が与えられるタイミングは，通路分類モジュールの出力が変化したタイミングとなる．
つまり通路の分類が遅れることで目標方向を与えるタイミングが遅くなり，曲がることができないと考えた．
...
\begin{figure*}[htbp]
    \begin{tabular}{ccc}
        \begin{minipage}[t]{0.5\textwidth}
            \centering
            \includegraphics[keepaspectratio, width=60mm]{images/png/ishiguro/learning_1.png}
            \subcaption{Location of label changes during learning}
        \end{minipage} &
        \begin{minipage}[t]{0.5\textwidth}
            \centering
            \includegraphics[keepaspectratio, width=60mm]{images/png/ishiguro/test_1.png}
...
\begin{figure*}[htbp]
    \begin{tabular}{ccc}
        \begin{minipage}[t]{0.5\textwidth}
            \centering
            \includegraphics[keepaspectratio, width=60mm]{images/png/ishiguro/learning_2.png}
            \subcaption{Location of label changes during learning}
        \end{minipage} &
        \begin{minipage}[t]{0.5\textwidth}
            \centering
            \includegraphics[keepaspectratio, width=60mm]{images/png/ishiguro/test_2.png}
            \subcaption{Location of label changes during testing}
        \end{minipage}
    \end{tabular}
